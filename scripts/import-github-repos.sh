...
import_repo() {
    ...
    git clone --depth 1 --quiet "$url" "$temp_dir" </dev/null 2>/dev/null && {
...
# Read repos into array to avoid stdin consumption issues
mapfile -t REPO_LINES < "$REPO_LIST_FILE"

log_info "Loaded ${#REPO_LINES[@]} lines from repo list"

for line in "${REPO_LINES[@]}"; do
    # Parse the line
    IFS='|' read -r org name url target_dir status <<< "$line"
