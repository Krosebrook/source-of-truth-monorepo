...
async function healthCheck() {
  ...
  try {
    ...
    if (apiKeyService && apiKeyService.SecureApiKeyService) {
      ...
      log(
        `  ${configValid ? "✅" : "⚠️"} API Key Service: ${configValid ? "Healthy" : "Degraded"}`,
        configValid ? "green" : "yellow"
      );
    } else {
      services.push({
        name: "API Key Service",
  ...
  try {
    ...
    if (aiService && aiService.SecureAIService) {
      ...
      const statusIcon =
        aiHealth.status === "healthy" ? "✅" : aiHealth.status === "degraded" ? "⚠️" : "❌";
      const statusColor =
        aiHealth.status === "healthy" ? "green" : aiHealth.status === "degraded" ? "yellow" : "red";
  ...
  if (process.env.SUPABASE_URL && process.env.SUPABASE_ANON_KEY) {
    try {
      ...
      log(
        `  ${dbHealthy ? "✅" : "❌"} Supabase: ${dbHealthy ? "Connected" : "Connection failed"}`,
        dbHealthy ? "green" : "red"
      );
      totalServices++;
  ...
  log(
    `  Services: ${healthyServices}/${totalServices} healthy (${Math.round(overallHealth * 100)}%)`,
    statusColor
  );
