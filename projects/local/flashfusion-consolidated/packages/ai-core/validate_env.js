...
function validateEnvironment() {
  ...
  const requiredVars = ["OPENAI_API_KEY", "ANTHROPIC_API_KEY", "SUPABASE_URL", "SUPABASE_ANON_KEY"];

  const optionalVars = ["LANGSMITH_API_KEY", "GITHUB_TOKEN", "GEMINI_API_KEY", "REPLIT_API_TOKEN"];
  ...
  if (process.env.OPENAI_API_KEY && !process.env.OPENAI_API_KEY.includes("your_")) {
    ...
    log(
      `  ${isValid ? "✅" : "❌"} OpenAI API Key format: ${isValid ? "Valid" : "Invalid"}`,
      isValid ? "green" : "red"
    );
    if (!isValid) errors++;
  }
  ...
  if (process.env.ANTHROPIC_API_KEY && !process.env.ANTHROPIC_API_KEY.includes("your_")) {
    ...
    log(
      `  ${isValid ? "✅" : "❌"} Anthropic API Key format: ${isValid ? "Valid" : "Invalid"}`,
      isValid ? "green" : "red"
    );
    if (!isValid) errors++;
  }
