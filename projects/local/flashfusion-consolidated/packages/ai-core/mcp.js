...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (req.method === "POST" && path === "/start") {
      ...
      req.on("end", async () => {
        try {
          const { serverName, options = {} } = JSON.parse(body || "{}");
    ...
    if (req.method === "POST" && path === "/stop") {
      ...
      req.on("end", async () => {
        try {
          const { serverName, force = false } = JSON.parse(body || "{}");
    ...
    if (req.method === "POST" && path === "/request") {
      ...
      req.on("end", async () => {
        try {
          const { serverName, method, params = {}, timeout } = JSON.parse(body || "{}");
          ...
          return res.mcpSuccess(response, `Request to ${serverName}.${method} completed`);
        } catch (error) {
          return res.mcpError(error.message, 500, {
            serverName: serverName || "unknown",
