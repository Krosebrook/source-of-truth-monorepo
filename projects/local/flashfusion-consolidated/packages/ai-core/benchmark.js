...
class FlashFusionBenchmark {
  ...
  async benchmarkHealthCheck() {
    ...
    const avg =
      this.results.health_check.reduce((a, b) => a + b, 0) / this.results.health_check.length;
  ...
  async benchmarkStatusCheck() {
    ...
    const avg =
      this.results.status_check.reduce((a, b) => a + b, 0) / this.results.status_check.length;
    console.log(`\n   Average: ${avg.toFixed(2)}ms`);
  }
  ...
  async benchmarkAgentAnalysis() {
    ...
    const avg =
      this.results.agent_analysis.reduce((a, b) => a + b, 0) / this.results.agent_analysis.length;
    console.log(`\n   Average: ${avg.toFixed(2)}ms`);
  }
  ...
  async benchmarkConcurrentRequests() {
    ...
    for (let i = 0; i < concurrentCount; i++) {
      promises.push(axios.get(`${BASE_URL}/health`).catch(() => {}));
    }
