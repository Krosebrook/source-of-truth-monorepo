        ...
        try {
          if (res.statusCode === 200) {
            ...
            if (
              ...
              response.data.currentUser.publicRepls
            ) {
...
function updateEnvWithReplit(apiToken) {
  try {
    ...
    envContent = envContent.replace(/REPLIT_API_TOKEN=.*/, `REPLIT_API_TOKEN=${apiToken}`);
...
async function main() {
  ...
  console.log("────────────────────────────────────────────────────");
  log(
    connection.success ? "green" : "yellow",
    `${connection.success ? "✅" : "⚠️"} Replit API: ${connection.success ? "Connected" : "Token configured"}`
  );
