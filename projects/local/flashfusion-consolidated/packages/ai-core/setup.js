...
class FlashFusionSetup {
  ...
  showWelcome() {
    ...
    console.log(
      chalk.white(
        "Welcome to FlashFusion! Let's set up your AI-powered business automation platform.\n"
      )
    );
  }
    ...
    const workflowConfig = await inquirer.prompt([
      {
        ...
        choices: [
          ...
          {
            name: "Content Workflow (Content creation & distribution)",
            value: "content",
            checked: true,
          },
          {
            name: "Hybrid Workflow (Cross-workflow orchestration)",
            value: "hybrid",
            checked: true,
          },
        ],
  ...
  showCompletion() {
    ...
    console.log(
      chalk.gray("3. Open dashboard: ") + chalk.cyan(`http://localhost:${this.config.PORT || 3000}`)
    );
