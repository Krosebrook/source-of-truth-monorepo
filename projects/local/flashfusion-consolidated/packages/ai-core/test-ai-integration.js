    ...
    messages: [
      {
        role: "user",
        content:
          "Test FlashFusion AI integration. Respond with: 'FlashFusion AI integration successful!'",
      },
    ],
    max_tokens: 50,
    temperature: 0.7,
...
async function main() {
  ...
  log(
    "cyan",
    `  Key Format: ${OPENAI_API_KEY.startsWith("sk-proj") ? "Project Key ✅" : "Standard Key"}`
  );
  ...
  log(
    aiServiceWorking ? "green" : "yellow",
    `${aiServiceWorking ? "✅" : "⚠️"} FlashFusion AI Service`
  );
  log(
    agentIntelligence ? "green" : "yellow",
    `${agentIntelligence ? "✅" : "⚠️"} Agent Intelligence`
  );
