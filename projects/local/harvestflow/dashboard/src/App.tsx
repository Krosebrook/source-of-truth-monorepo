  ...
  return (
    <main style={{ fontFamily: "system-ui", margin: "2rem" }}>
      ...
      <table style={{ width: "100%", borderCollapse: "collapse" }}>
        <thead>
          <tr>
            {["Flow", "Title", "qualityScore", "Success", "Efficiency", "Automated", "User"].map(
              (label) => (
                <th
                  key={label}
                  style={{ textAlign: "left", padding: "0.5rem", borderBottom: "1px solid #ccc" }}
                >
                  {label}
                </th>
              )
          ...
          {filtered.map((flow) => (
            <tr key={flow.flowId}>
              ...
              <td style={{ padding: "0.5rem", borderBottom: "1px solid #eee" }}>
                {flow.qualityScore.toFixed(3)}
              </td>
              <td style={{ padding: "0.5rem", borderBottom: "1px solid #eee" }}>
                {flow.metrics.taskSuccessRate.toFixed(2)}
              </td>
              <td style={{ padding: "0.5rem", borderBottom: "1px solid #eee" }}>
                {flow.metrics.efficiencyScore.toFixed(2)}
              </td>
              <td style={{ padding: "0.5rem", borderBottom: "1px solid #eee" }}>
                {flow.metrics.automatedGateScore.toFixed(2)}
              </td>
              <td style={{ padding: "0.5rem", borderBottom: "1px solid #eee" }}>
                {flow.metrics.userRating.toFixed(2)}
              </td>
            </tr>
          ))}
        </tbody>
      ...
      {filtered.length === 0 && (
        <p style={{ marginTop: "1.5rem" }}>No flows match the current filters.</p>
      )}
    </main>
  );
}
